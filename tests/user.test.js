...
test('Should signup a new user', async () => {
    const response = await request(app).post('/users').send({
        ...
        password: 'lightyearsahead'
    }).expect(201)

    // Asserting database was changed
    ...
    expect(user.password).not.toBe('lightyearsahead')
})
...
test('Should login existing user', async () => {
    ...
    // Asserting that the token for logged in user matches userOne's token
...
test('Should update valid user fields', async () => {
    ...
    const user = await User.findById(userOneId)
    expect(user.name).toEqual('Mister1')
})
